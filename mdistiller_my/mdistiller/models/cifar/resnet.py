...
if __name__ == "__main__":
    ...
    logit, feats, fc= net(x)

    for f in feats["feats"]:
        print(f.shape, torch.take(f, torch.tensor([0, 1, 2])))
    print(feats["pooled_feat"].shape)
    print(fc)
    print(logit.shape)
    a = feats["feats"][1].pow(2).mean(1).view(feats["feats"][1].size(0), -1)
    b = F.normalize(a)
    print(feats["feats"][1].shape, a.shape, b.shape)
