...
class TEST(Distiller):
    ...
    def __init__(self, student, teacher, cfg):
        ...
        self.task_loss_weight = cfg.TEST.LOSS.CE_WEIGHT
        self.kl_loss_weight = cfg.TEST.LOSS.KD_WEIGHT
        self.tat_loss_weight = cfg.TEST.LOSS.TAT_WEIGHT
        self.tat2_loss_weight = cfg.TEST.LOSS.TAT2_WEIGHT
    ...
    def forward_train(self, image, target, **kwargs):
        ...
        #f_s_l = self.conv1(f_s)
        #f_s_l = self.bn1(f_s_l)
        #f_s_o = self.conv3(f_s)
        #f_s_o = self.bn3(f_s_o)
        f_t_l = self.conv2(f_t)
        f_t_l = self.bn2(f_t_l)
        #f_s_o1 = do_patches(f_s_o)
        #f_s_l1= do_patches(f_s_l)
        #f_t_l1 = do_patches(f_t_l)
        ...
        loss_feat = self.tat_loss_weight * single_stage_tat_loss(
            f_s, f_t_l, f_s
        )
